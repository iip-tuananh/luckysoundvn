				...
				<header class="entry-header">
					<div class="entry-header-text entry-header-text-top text-left">
						<h6 class="entry-category is-xsmall">
							<a href="{{route('listService')}}" rel="category tag">Dịch vụ</a>
						</h6>
						<h1 class="entry-title">{{($detail_service->name)}}</h1>
				...
				<div class="bai-viet-lien-quan">
					...
					<ul class="list-bai-viet">
						@foreach ($services as $service)
							<li>
								<div class="box-image">
								<a href="{{route('serviceDetail', ['slug'=>$service->slug])}}"><img width="700" height="440" src="{{$service->image}}" class="attachment-large size-large wp-post-image" alt="{{$service->name}}" loading="lazy" srcset="{{$service->image}} 700w, {{$service->image}} 600w" sizes="(max-width: 700px) 100vw, 700px" /></a>
								</div>
								<a href="{{route('serviceDetail', ['slug'=>$service->slug])}}">
								<h4 class="tieu-de-bai-viet">{{$service->name}}</h4>
								</a>
							</li>
						@endforeach
					</ul>
